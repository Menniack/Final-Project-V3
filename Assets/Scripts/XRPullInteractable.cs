...
namespace UnityEngine.XR.Interaction.Toolkit.Interactables
{
    public class XRPullInteractable : XRBaseInteractable
    {
        ...
        public override void ProcessInteractable(XRInteractionUpdateOrder.UpdatePhase updatePhase)
        {
            ...
            // Make sure it's the Dynamic phase, as this is the phase when you're able to modify the object when
            // being interacted with
            if (updatePhase == XRInteractionUpdateOrder.UpdatePhase.Dynamic)
            {
                if (isSelected && _pullInteractor != null)
                {
                    Vector3 pullPosition = _pullInteractor.transform.position;
        ...
        {
            Vector3 linePosition = Vector3.forward * Mathf.Lerp(_startPoint.transform.localPosition.z, _endPoint.transform.localPosition.z, pullAmount);
            _notchPoint.transform.localPosition = linePosition;
            _lineRenderer.SetPosition(1, linePosition);
        }

        
        // Some fun haptic stuff!
        private void HandleHaptics()
        {
            if (_pullInteractor != null && _pullInteractor is XRBaseInputInteractor controllerInteractor)
            {
